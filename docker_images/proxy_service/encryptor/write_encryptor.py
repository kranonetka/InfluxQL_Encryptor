...
class WriteTokensEncryptor(Encryptor):
    ...
    def encrypt(self, tokens, db):
        ...
        for tag_key in tokens.get("tags").keys():
            tag_value = tokens.get("tags").get(tag_key)
            encrypted_tag_value = self.encrypt_bytes(tag_value.encode())
        ...
        for field_key in tokens.get("fields").keys():
            ...
            if type_of_value == "string":
                encrypted_field_value = self.encrypt_bytes(field_value.encode())
